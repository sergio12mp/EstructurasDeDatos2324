...
-- Si el elemento es el primero de su equipo, se añade la final de la cola.
 ...
 mkTQ e lista = foldl (\q x -> enqueue x e q) Empty lista
 ...
 filterTQ f cola = foldrTQ (\x acc -> if f x then Node x acc else acc) Empty cola
...
firstTeam n Empty = []
firstTeam n (Node x q) = go (x `mod` n) [x] q
    where
        go _ acc Empty = acc -- Devuelve la lista acumulada cuando se llega al final de la cola
        go equipo acc (Node y rest)
        | equipo == y `mod` n = go equipo (acc ++ [y]) rest -- Sigue acumulando elementos mientras sean del mismo equipo
        | otherwise = acc -- Cuando se encuentra un equipo distinto, se detiene el proceso y devuelve la lista acumulada

 -- dequeueTeam. Desencola el primer equipo de la cola de equipos
 -- 1.00 Implementación recursiva
 dequeueTeam :: Equipos -> TQueue -> TQueue
 dequeueTeam e cola = removeFirstTeam (firstTeam e cola) cola
 -- (10,4,2,8,6) -> [10,4]
 removeFirstTeam :: [Integer] -> TQueue -> TQueue
 removeFirstTeam _ Empty = Empty
 removeFirstTeam y (Node x q)
 | x `elem` y = removeFirstTeam y q -- Si el elemento está en la lista del primer equipo, lo omitimos

112 | otherwise = Node x (removeFirstTeam y q) -- Si no está en la lista del primer equipo,
lo mantenemos
113
114 -- getTeam. Devuelve una lista con los elementos de un equipo t que están en la cola de n
equipos
115 -- El primer argumento es el equipo que se busca (t) y el segundo el número de equipos de
la cola (n)
